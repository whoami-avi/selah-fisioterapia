...
// Supabase credentials - Configured from system secrets
...
// ===== THEME MANAGEMENT =====
function initTheme() {
    const savedTheme = localStorage.getItem('selah_theme') || 'light';
...
function toggleTheme() {
    ...
    applyTheme(newTheme);
    localStorage.setItem('selah_theme', newTheme);
    showToast(`Tema: ${newTheme === 'dark' ? 'Oscuro' : 'Claro'}`, 'success');
}
...
const VERSION_HISTORY = [
    { version: '3.0.0', date: '2026-04-17', notes: 'Modo Claro/Oscuro, Mejoras en sincronización, Corrección de bugs' },
...
function initSupabase() {
    ...
    console.log('🔧 initSupabase - URL:', url);
    console.log('🔧 initSupabase - Key existe:', !!key);
    console.log('🔧 initSupabase - window.supabase existe:', !!window.supabase);
...
async function loadDataFromSupabase() {
    ...
    if (!supabaseClient) {
        console.log('❌ No hay supabaseClient, usando localStorage');
    ...
    try {
        ...
        templates = templatesData && templatesData.length > 0 ? templatesData : getDefaultTemplates();
        ...
        isDataLoaded = true;
        console.log('✅ Datos cargados desde Supabase - Pacientes:', patients.length, 'Citas:', appointments.length);
...
function loadFromLocalStorage() {
    patients = JSON.parse(localStorage.getItem('selah_patients')) || [];
    appointments = JSON.parse(localStorage.getItem('selah_appointments')) || [];
    templates = JSON.parse(localStorage.getItem('selah_templates')) || getDefaultTemplates();
    pendingChanges = JSON.parse(localStorage.getItem('selah_pending')) || [];
    messageHistory = JSON.parse(localStorage.getItem('selah_messages')) || [];
    isDataLoaded = true;
    console.log('📱 Datos cargados desde localStorage (offline)');
}
...
function setupRealtimeSubscriptions() {
    ...
    supabaseClient
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'templates' }, (payload) => {
            console.log('📊 Templates change:', payload);
            handleRealtimeChange('templates', payload);
        })
        .subscribe();
    ...
    supabaseClient
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, (payload) => {
            ...
            handleRealtimeChange('messages', payload);
        })
        .subscribe();
...
function handleRealtimeChange(table, payload) {
    ...
    switch (eventType) {
        case 'INSERT':
            ...
            } else if (table === 'messages' && !messageHistory.find(m => m.id === newRecord.id)) {
                messageHistory.unshift(newRecord);
            }
            break;
        ...
        case 'UPDATE':
            if (table === 'patients') {
                ...
                const mIndex = messageHistory.findIndex(m => m.id === newRecord.id);
                if (mIndex !== -1) messageHistory[mIndex] = newRecord;
            }
            break;
            
        case 'DELETE':
            if (table === 'patients') {
                patients = patients.filter(p => p.id !== oldRecord.id);
            } else if (table === 'appointments') {
                appointments = appointments.filter(a => a.id !== oldRecord.id);
            } else if (table === 'templates') {
                templates = templates.filter(t => t.id !== oldRecord.id);
            } else if (table === 'messages') {
                messageHistory = messageHistory.filter(m => m.id !== oldRecord.id);
            }
            break;
    }
...
function refreshCurrentSection() {
    ...
    } else if (sectionId === 'whatsapp' || sectionId === 'templates') {
        if (typeof renderTemplates === 'function') renderTemplates();
        if (typeof renderMessageHistory === 'function') renderMessageHistory();
    } else if (sectionId === 'finance' || sectionId === 'finances') {
...
async function updatePatientInSupabase(patientId, updates) {
    // IMPORTANT: Remove 'id' from updates to avoid PostgreSQL error
    ...
    try {
        console.log('🔄 Actualizando paciente:', patientId, 'con datos:', updateData);
        const { data, error } = await supabaseClient
            .from('patients')
            .update(updateData)
            .eq('id', patientId)
...
async function saveAppointmentToSupabase(appointmentData) {
    if (!supabaseClient) {
        appointments.push(mapAppointmentFromSupabase(appointmentData));
        appointments.sort((a, b) => new Date(a.date) - new Date(b.date));
        saveToLocalStorage();
    ...
    try {
        ...
        // Avoid duplicates if the realtime listener already pushed it
        if (!appointments.find(a => a.id === mappedData.id)) {
        ...
        // Map to client-side shape so the dashboard can render it even on fallback
        const mapped = mapAppointmentFromSupabase(appointmentData);
...
async function updateAppointmentInSupabase(appointmentId, updates) {
...
async function updateMessageInSupabase(messageId, updates) {
    if (!supabaseClient) {
        const index = messageHistory.findIndex(m => m.id === messageId);
        if (index !== -1) {
            messageHistory[index] = { ...messageHistory[index], ...updates };
        }
        saveToLocalStorage();
...
function handleLogout() {
    ...
    if (confirm('¿Deseas cerrar sesión?')) {
        ...
        // Clear any cached patient/appointment data
        patients = [];
        appointments = [];
        ...
        // Force hide app container and show login
        document.getElementById('appContainer').style.display = 'none';
        document.getElementById('loginScreen').style.display = 'flex';
...
async function initializeApp() {
    ...
    localStorage.removeItem('selah_patients');
    localStorage.removeItem('selah_appointments');
...
async function savePatient(event) {
    ...
    const patientData = {
        ...
        occupation: document.getElementById('patientOccupation').value || null,
        referrer: document.getElementById('patientReferrer').value || null,
        address: document.getElementById('patientAddress').value || null,
        diagnosis: document.getElementById('patientDiagnosis').value || null,
        treatment: document.getElementById('patientTherapyType').value || null, // Map therapy_type to treatment column
        notes: document.getElementById('patientNotes').value || null,
        emergency_contact: document.getElementById('patientEmergencyContact')?.value || null,
        emergency_phone: document.getElementById('patientEmergencyPhone')?.value || null,
        created_at: existingId ? (patients.find(p => p.id === existingId)?.created_at || new Date().toISOString()) : new Date().toISOString(),
        updated_at: new Date().toISOString()
    };

    // Use Supabase (or fallback to local)
    if (existingId) {
        console.log('📝 Actualizando paciente existente:', id);
        await updatePatientInSupabase(id, patientData);
    } else {
        console.log('➕ Guardando nuevo paciente:', id);
...
function deletePatient() {
    ...
    if (confirm('¿Eliminar este paciente? Esta acción no se puede deshacer.')) {
        ...
        appointments = appointments.filter(a => a.patient_id !== id && a.patientId !== id);
        saveToLocalStorage();
...
async function saveAppointment(event) {
    ...
    const apptData = {
        ...
        type: document.getElementById('appointmentTherapy').value,
...
async function deleteAppointment() {
    ...
    if (confirm('¿Eliminar esta cita?')) {
        // Eliminar optimistamente del array local primero para reflejar en la UI
...
function saveTemplate(event) {
    ...
    const existingId = document.getElementById('templateId').value;
    const id = existingId || crypto.randomUUID();
    ...
    const templateData = {
        ...
        created_at: existingId ? (templates.find(t => t.id === existingId)?.created_at || new Date().toISOString()) : new Date().toISOString()
    };
    
    // Use Supabase (or fallback to local)
    saveTemplateToSupabase(templateData);
